...
wss.on("connection", function(ws) {
  ...
  ws.on("message", function(msg) {
    if (!ws.clientConfig.allDataPassed) {
      ...
      if (!ws.clientConfig.targetPass) { // Check if we have the password for the account
      ...
      switch (data.mode) {
        ...
        case "listen": {
          if (!data.port) return ws.send(JSON.stringify({
            success: false,
            error: "Missing port"
          }))

          if (data.port <= 0 || data.port > 65535) return ws.send(JSON.stringify({
            success: false,
        ...
        case "connect": {
          ...
          if (!data.hostID) return ws.send(JSON.stringify({
            success: false,
          ...
          if (data.port <= 0 || data.port > 255) return ws.send(JSON.stringify({
            success: false,
